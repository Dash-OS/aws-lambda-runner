...
```
yarn add aws-lambda-runner@next
```

**_OR_**

```
npm install --save aws-lambda-runner@next
```
...
export default run((body, config) => ({
	hello: "world"
}));
```
...
```json
{
	...
	"client": {
		...
		"forwardedFor": ["<masked-ip>", "<masked-ip>"],
		"forwardedPort": 443,
		"country": "US",
		"sourceIP": "<masked-ip>"
	},
