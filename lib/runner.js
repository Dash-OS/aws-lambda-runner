  ...
  if ( isLogLevel(config, 'debug') ) {
    handleLogDebug(
        ...
        : 'Returning Response to Caller:',
      {
        ...
        body: tryStringifyJSON(result, null, 2),
        headers: {
          ...config.cors 
    ...
    try {
      ...
      const plugins = config.plugins.map(plugin => {
        ...
        if ( Array.isArray(plugin) ) {
          ...
          const [ pclass, ...plugin_settings ] = plugin
          p = new pclass(...plugin_settings)
        } else {
          p = new plugin()
        }
        ...
        return Promise.resolve(val).then(r => p)
      })
      return Promise.all(plugins).then(plugins => {
        ...
        return Promise.resolve(fn(data, config, ctx, cb))
          .then(result => {
            const plugin_promises = plugins.map(plugin => {
              if ( typeof plugin.onComplete === 'function' ) {
                return plugin.onComplete(result, data, config, ctx, cb)
              }
            })
            return Promise.all(plugin_promises).then(() => {
