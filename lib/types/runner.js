...
export interface RunnerPluginInterface<PS> {
  pluginID: string;
  settings: PS;
...
*/
export type RunnerUserConfig<PS: Object> = {|
  settings?: {|
  ...
  |},
  plugins?: Array<Class<RunnerPlugin<PS>> | [Class<RunnerPlugin<PS>>, PS]>
|};

export type RunnerConfig<PS: Object> = {|
  settings: {
    isBase64Encoded?: boolean | false,
    log: boolean | 'debug',
    successCode?: number | 200,
  ...
  request: {
    ...
    headers: void | { [headerName: string]: string },
    method: void | 'DELETE' | 'GET' | 'HEAD' | 'OPTIONS' | 'PATCH' | 'POST' | 'PUT',
    queries: {
...
export type RunnerRuntimeConfig<PS> = $ReadOnly<{
  ...RunnerConfig<PS>,
  plugins: RunnerPlugin<PS>[]
}>;

export type RunnerTopConfig<PS> = {
  config: $Shape<RunnerConfig<PS>>
};

export type RunnerFunction = (() => any) &
  (<D: *>(data: D) => any) &
  (<D: *, C: RunnerRuntimeConfig<*>>(data: D, config: C) => any) &
  (<D: *, C: RunnerRuntimeConfig<*>, T: LambdaContextObject>(
    data: D,
    config: C,
    context: T,
  ) => any) &
  (<D: *, C: RunnerRuntimeConfig<*>, T: LambdaContextObject, F: LambdaCallbackFunction>(
    data: D,
    config: C,
    context: T,
    callback: F,
